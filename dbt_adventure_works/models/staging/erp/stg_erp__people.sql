with
    source_people as (
        select
            ...
            , to_number(to_char(cast(left(xmlget(DEMOGRAPHICS, 'DateFirstPurchase'):"$", 10) as date), 'YYYYMMDD')) as date_first_purchase
            , to_number(to_char(cast(left(xmlget(DEMOGRAPHICS, 'BirthDate'):"$", 10) as date), 'YYYYMMDD')) as date_birth
